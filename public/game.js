...
const BASE_DRIVE_SPEED = 1.2; // wheel angular velocity target — bumped up to compensate for the heavier gravity
...
const DRIVE_RESPONSE_PER_FRAME = 0.3; // how fast the wheel's spin ramps toward its target, per animation frame
...
function mountWheels(car, points, features) {
  ...
  const wheels = offsets.map(off => {
    const w = Bodies.fromVertices(cx + off.x, cy + off.y, [vertices], {
      friction: 0.9, frictionStatic: 1.0, density: 0.0035, restitution: 0,
