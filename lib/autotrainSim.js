...
function buildGround(world, terrainType, conditions) {
  ...
  switch (terrainType) {
    ...
    case 'sand':
      ...
      break;
    case 'water':
      // Recessed floor, not a same-height box — see WATER_PIT_DEPTH note in
      // public/script.js's buildGroundBodies 'water' case.
      addBox((tx0 + tx1) / 2, baseY + WATER_PIT_DEPTH + 20, SEG_LEN, 40, 0, conditions.water.friction, 'water');
      break;
    case 'ice':
...
async function runTrial(terrainType, wheelPoints, widthScale) {
  ...
  while (simMs < MAX_SIM_MS) {
    ...
    if (onTerrain && terrainType === 'water') {
      ...
      const equilibriumDepth = 46 / displacement;
      const targetY = GROUND_Y + equilibriumDepth;
      const wdy = (targetY - chassis.position.y) * 0.08;
