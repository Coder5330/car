...
const BASE_DRIVE_SPEED = 0.5;
const PHYSICS_SUBSTEPS = 4;
const DRIVE_RESPONSE = 0.16;
...
// an array of { wheelPoints: [[x,y],...], score } already fetched by the
// caller (server.js) — this module has no DB access of its own.
function pickWheel(candidates) {
  if (!candidates || candidates.length === 0) {
    return { points: coldStartWheel(), reason: 'no examples yet — cold-start' };
  }
  if (Math.random() <= 0.15) {
    return { points: coldStartWheel(), reason: `cold-start (random roll, ${candidates.length} available)` };
  }
  ...
  return { points, reason: `imitating score-${pick.score} wheel (${candidates.length} available)` };
}
...
function buildGround(world, terrainType, conditions) {
  ...
  addBox((x0 + x1) / 2, baseY + 20, x1 - x0, 40, 0, 0.7, 'flat');
  ...
  switch (terrainType) {
    ...
    case 'sand':
      ...
      break;
    case 'water':
      addBox((tx0 + tx1) / 2, baseY + 20, SEG_LEN, 40, 0, conditions.water.friction, 'water');
      break;
    case 'ice':
  ...
  addBox((ex0 + ex1) / 2, baseY + 20, ex1 - ex0, 40, 0, 0.7, 'flat');
...
async function runTrial(terrainType, wheelPoints) {
  ...
  const wheels = offsets.map(off => {
    const w = Bodies.fromVertices(startX + off.x, GROUND_Y - 60 + off.y, [vertices], {
      friction: 0.85, frictionStatic: 1.0, density: 0.0035, restitution: 0, collisionFilter: filter
    }, true);
  ...
  while (simMs < MAX_SIM_MS) {
    ...
    if (onTerrain && terrainType === 'sand') {
      const wideness = Math.max(0, Math.min(1, (features.widthRatio - conditions.sand.wideBias) / 1.2));
    ...
    if (onTerrain && terrainType === 'water') {
      Body.applyForce(chassis, chassis.position, { x: 0, y: -conditions.water.buoyancy * chassis.mass });
    }
