...
// Re-orders points by angle around their own centroid — see aiGenerateWheel
// in public/script.js for why (independent per-point jitter can turn a
// simple outline self-intersecting, which breaks the client's 3D wheel mesh
// even though poly-decomp physics tolerates it fine). This file has no
// renderer, but examples stored here get imitated by the live client later,
// so keep stored wheel_points in the same well-ordered state either way.
...
// Structured random wheel — mirrors public/script.js's coldStartWheel
// exactly (see the comment there for why per-vertex noise was replaced).
// Keep in sync: this generates the shapes autotrain scores and stores, so
// drift here means the AI learns from a different design space than the
// live game explores.
...
function pickWheel(candidates) {
  ...
  const jitter = 6;
  // Explicit size mutation alongside the point jitter — see the client's
  // aiGenerateWheel for why jitter alone can't evolve size. Keep in sync.
  ...
  while (simMs < MAX_SIM_MS) {
    ...
    if (onTerrain && terrainType === 'ice') {
      ...
      if (smoothSpeed > ICE_SAFE_SPEED) {
        ...
        // Spin fades out as the car tilts so ice is a recoverable wobble,
        // not a certain flip — see public/script.js for the full reasoning
        // and the measurements. Keep in sync.
        const iceTilt = Math.abs(Math.atan2(Math.sin(chassis.angle), Math.cos(chassis.angle)));
        const tiltFade = Math.max(0, 1 - iceTilt / 1.45);
        Body.setAngularVelocity(chassis, chassis.angularVelocity + iceSlipDir * severity * 0.012 * PHYSICS_SUBSTEPS * tiltFade);
      } else {
        iceSlipDir = null;
      }
    }
